 ...
 */
public class HLAinfrastructureSystem extends HLAobject {
	...
	/**
	 * Creates the local infrastructure system.
	 *
	 * @param rtiAmbassador the rti ambassador
	 * @param encoderFactory the encoder factory
	 * @param localSystem the local system
	 * @return the HLA infrastructure system
	 ...
	 */
	public static HLAinfrastructureSystem createLocalInfrastructureSystem(
			RTIambassador rtiAmbassador, EncoderFactory encoderFactory,
			InfrastructureSystem.Local localSystem) 
		...
		HLAinfrastructureSystem hlaSystem = new HLAinfrastructureSystem(
				rtiAmbassador, encoderFactory, localSystem);
		localSystem.addAttributeChangeListener(hlaSystem);
		return hlaSystem;
	}
	 ...
	 * @throws RestoreInProgress the restore in progress
	 */
	public static HLAinfrastructureSystem createRemoteInfrastructureSystem(
			RTIambassador rtiAmbassador, EncoderFactory encoderFactory,
			String instanceName, InfrastructureSystem.Remote remoteSystem) 
					throws NameNotFound, FederateNotExecutionMember, 
					NotConnected, RTIinternalError, InvalidObjectClassHandle, 
					ObjectInstanceNotKnown, AttributeNotDefined, SaveInProgress, 
					RestoreInProgress {
		HLAinfrastructureSystem hlaSystem = new HLAinfrastructureSystem(
				rtiAmbassador, encoderFactory, remoteSystem);
	...
	protected final InfrastructureSystem infrastructureSystem;
	protected final HLAunicodeString name;
	protected final HLAunicodeString societyName;
	protected final HLAfloat64BE netCashFlow;
	protected final HLAfloat64BE domesticProduction;
	...
	public void attributeChanged(AttributeChangeEvent evt) {
		if(evt.getSource().equals(infrastructureSystem)) {
			// TODO need to trigger events from local model...
